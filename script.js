...
function createDynamicParticles() {
  ...
  // Create many more particles for dramatic warp effect
  for (let i = 0; i < 120; i++) {
    ...
    const distance = Math.random() * 1200 + 400; // Much further distance
    const startDistance = Math.random() * 20 + 5; // Closer to center start
    ...
    let size, opacity, duration, glowColor;
    
    if (particleType < 0.6) {
      // Regular particles - most common
      ...
      // Golden/orange particles - rarest
      size = Math.random() * 5 + 3;
      opacity = Math.random() * 0.9 + 0.5;
      duration = Math.random() * 3 + 1;
      glowColor = 'rgba(255,200,100,';
    }
    ...
    particle.style.cssText = `
      ...
      width: ${size}px;
      height: ${size}px;
      background: radial-gradient(circle, ${glowColor}${opacity}) 0%, ${glowColor}${opacity * 0.6}) 40%, transparent 70%);
      border-radius: 50%;
      left: calc(50% + ${startX}px);
      top: calc(50% + ${startY}px);
      animation: particle-warp-intense ${duration}s ease-out infinite;
      animation-delay: -${delay}s;
      --end-x: ${deltaX}px;
      --end-y: ${deltaY}px;
      --start-size: ${size}px;
      box-shadow: 0 0 ${size * 2}px ${glowColor}${opacity * 0.5});
  ...
  // Add some streaking particles for extra drama
  for (let i = 0; i < 15; i++) {
    const streak = document.createElement('div');
    streak.className = 'streak-particle';
    ...
    streak.style.cssText = `
      ...
      animation: streak-warp ${duration}s ease-out infinite;
      animation-delay: -${delay}s;
      --end-x: ${deltaX}px;
      --end-y: ${deltaY}px;
    `;
  ...
  // Add CSS animations for intense warp speed effect
  const style = document.createElement('style');
  style.textContent = `
    @keyframes particle-warp-intense {
      0% {
      ...
      15% {
        transform: translate(calc(var(--end-x) * 0.05), calc(var(--end-y) * 0.05)) scale(0.2);
        filter: blur(0px) brightness(2);
      }
      30% {
        transform: translate(calc(var(--end-x) * 0.15), calc(var(--end-y) * 0.15)) scale(0.4);
        filter: blur(0.5px) brightness(2.5);
      }
      50% {
        transform: translate(calc(var(--end-x) * 0.4), calc(var(--end-y) * 0.4)) scale(0.8);
      ...
      85% {
        transform: translate(calc(var(--end-x) * 0.9), calc(var(--end-y) * 0.9)) scale(2.5);
        filter: blur(3px) brightness(2);
        opacity: 0.7;
      }
      95% {
        transform: translate(var(--end-x), var(--end-y)) scale(4);
        filter: blur(4px) brightness(1.5);
        opacity: 0.3;
      }
      100% {
        transform: translate(var(--end-x), var(--end-y)) scale(5);
    ...
    @keyframes streak-warp {
      0% {
        transform: translate(0px, 0px) rotate(var(--angle)) scaleX(0.1);
        opacity: 0;
      }
      10% {
        opacity: 1;
        transform: translate(calc(var(--end-x) * 0.1), calc(var(--end-y) * 0.1)) rotate(var(--angle)) scaleX(0.3);
      }
      40% {
        opacity: 1;
        transform: translate(calc(var(--end-x) * 0.4), calc(var(--end-y) * 0.4)) rotate(var(--angle)) scaleX(1);
      }
      70% {
        opacity: 0.8;
        transform: translate(calc(var(--end-x) * 0.8), calc(var(--end-y) * 0.8)) rotate(var(--angle)) scaleX(2);
      }
      90% {
        opacity: 0.3;
        transform: translate(calc(var(--end-x) * 0.95), calc(var(--end-y) * 0.95)) rotate(var(--angle)) scaleX(3);
      }
      100% {
        opacity: 0;
        transform: translate(var(--end-x), var(--end-y)) rotate(var(--angle)) scaleX(4);
      }
    }
